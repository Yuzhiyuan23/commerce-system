...
async function sendCartRequest<T>(input: RequestInfo, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    // 详细显示验证错误
    let errorMessage = payload?.message ?? "请求失败，请稍后重试";
    if (response.status === 400 && payload?.message?.includes("Invalid request content")) {
      errorMessage = "请求内容验证失败，请检查所有必填字段是否填写正确";
    }
    throw new CartRequestError(response.status, errorMessage);
  }
