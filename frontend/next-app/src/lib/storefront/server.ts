...
export async function getServerStorefrontProductsByTag(
  tag: string,
  params: StorefrontListParams = {}
): Promise<StorefrontPagedResponse<StorefrontProductCard>> {
  const search = new URLSearchParams();
  ...
  return fetchStorefrontJson<StorefrontPagedResponse<StorefrontProductCard>>(`/api/products/by-tag?${search.toString()}`);
}
