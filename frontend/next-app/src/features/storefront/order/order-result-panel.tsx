...
import { cancelOrder, mockPayOrder } from "@/lib/order/client";
    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_0.95fr]">
      <article className="rounded-[30px] border border-black/10 bg-white/90 p-6 shadow-[0_18px_50px_rgba(74,42,18,0.08)]">
        ...
        <dl className="mt-6 grid gap-4 rounded-[24px] bg-[var(--surface)] p-4">
          ...
          <Metric label="订单ID" value={String(order.id)} />
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Link className="rounded-full border border-black/10 px-5 py-3 text-sm font-medium" href="/">
            返回主页
          </Link>
        </div>
      </article>
      ...
      <aside className="rounded-[30px] border border-black/10 bg-white/90 p-6 shadow-[0_18px_50px_rgba(74,42,18,0.08)]">
        <h2 className="text-2xl font-semibold tracking-tight">订单信息</h2>
        ...
        <div className="mt-6 flex flex-col gap-3">
          {canPay ? (
            <button
              className="rounded-full bg-[var(--accent)] px-5 py-3 text-center text-sm font-semibold text-white disabled:opacity-60"
              disabled={isPaying}
              type="button"
              onClick={() => {
                setMessage("");
                startPaying(async () => {
                  try {
                    console.log("[支付] 开始支付，订单ID:", order.id);
                    await mockPayOrder(order.id);
                    console.log("[支付] 支付成功，跳转到成功页面");
