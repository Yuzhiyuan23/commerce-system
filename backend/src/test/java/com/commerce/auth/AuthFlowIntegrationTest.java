...
import com.commerce.modules.user.service.PasswordService;
...
class AuthFlowIntegrationTest {
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        registry.add(
            ...
            () -> "jdbc:mysql://localhost:3306/commerce_system?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC");
        registry.add("spring.datasource.username", () -> "commerce");
        registry.add("spring.datasource.password", () -> "commerce123");
        registry.add("spring.flyway.enabled", () -> true);
        registry.add("spring.data.redis.host", () -> "localhost");
        registry.add("spring.data.redis.port", () -> 6379);
        registry.add("commerce.cache.enabled", () -> false);
        registry.add("commerce.rocketmq.enabled", () -> false);
    }
    ...
    void sessionPrincipalDoesNotRetainPasswordHashAfterSuccessfulLogin() throws Exception {
        ...
        assertThat(securityContext.getAuthentication().getPrincipal()).isInstanceOfSatisfying(
            com.commerce.modules.user.security.SessionUserPrincipal.class,
    ...
    void loginFailsWithWrongPassword() throws Exception {
        java.time.LocalDateTime beforeLogin = queryLastLoginAt("admin@commerce-system.local");

        mockMvc.perform(post("/api/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                    {
                      "email": "admin@commerce-system.local",
        ...
        assertLastLoginAt("admin@commerce-system.local", beforeLogin);
    }
