...
import static com.commerce.modules.order.dto.OrderDtos.OrderDetailResponse;
import static com.commerce.modules.order.dto.OrderDtos.OrderItemResponse;
import static com.commerce.modules.order.dto.OrderDtos.OrderStatusHistoryResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.AdminOrderListItemResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.AdminOrderListResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.AutoCompleteResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.ConfirmReceiptResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.ShipOrderResponse;
import static com.commerce.modules.order.dto.ShipmentDtos.ShipmentInfoResponse;

import java.time.LocalDateTime;
import java.util.List;

import com.commerce.modules.order.enums.OrderStatus;
import com.commerce.modules.order.enums.ShipmentStatus;
...
import com.commerce.modules.order.entity.ShipmentEntity;
...
import com.commerce.modules.order.mapper.ShipmentMapper;
...
public class ShipmentService {
    ...
    public OrderDetailResponse getAdminOrderDetail(Long orderId) {
        ...
        ShipmentInfoResponse shipment = getShipmentInfo(orderId);
        return toOrderDetailResponse(order, shipment);
    }

    @Transactional
    public OrderDetailResponse refundOrder(Long operatorUserId, Long orderId) {
        OrderEntity order = orderMapper.selectById(orderId);
        if (order == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Order not found");
        }
        // 只能退款已支付或已发货的订单
        if (!OrderStatus.PAID.name().equals(order.getOrderStatus())
                && !OrderStatus.SHIPPED.name().equals(order.getOrderStatus())) {
            throw new IllegalArgumentException("Only paid or shipped orders can be refunded");
        }

        String oldStatus = order.getOrderStatus();
        OrderEntity patch = new OrderEntity();
        patch.setOrderStatus(OrderStatus.CANCELLED.name());
        patch.setCancelledAt(LocalDateTime.now());
        patch.setCancelReason("商家退款");

        orderMapper.updateById(patch);

        insertOrderHistory(orderId, oldStatus, OrderStatus.CANCELLED.name(), operatorUserId, "商家退款");
