package com.commerce.modules.payment.service;

import static com.commerce.modules.payment.dto.PaymentDtos.PaymentAttemptResponse;
import static com.commerce.modules.payment.dto.PaymentDtos.PaymentActionResponse;
import static com.commerce.modules.payment.dto.PaymentDtos.PaymentOrderResponse;

import java.time.LocalDateTime;
import java.util.List;

import com.commerce.modules.payment.enums.PaymentStatus;
...
import com.commerce.modules.payment.mapper.PaymentMapper;
import com.commerce.modules.recommendation.GorseFeedbackService;
...
public class PaymentService {
    ...
    @Transactional
    public PaymentActionResponse mockPaySuccess(Long userId, Long orderId) {
        OrderEntity order = requireOwnedOrder(userId, orderId);

        if (OrderStatus.PAID.name().equals(order.getOrderStatus())) {
            // 已支付直接返回
            PaymentEntity existingPayment = paymentMapper.selectOne(
                new LambdaQueryWrapper<PaymentEntity>()
                    .eq(PaymentEntity::getOrderId, orderId)
                    .eq(PaymentEntity::getPaymentStatus, PaymentStatus.SUCCESS.name())
                    .orderByDesc(PaymentEntity::getId)
                    .last("limit 1"));
            if (existingPayment != null) {
                return toActionResponse(existingPayment, order);
            }
        }
        ...
        payment.setPaymentNo("MOCK-" + System.currentTimeMillis());
        payment.setPaymentMethod("MOCK");
        payment.setPaymentStatus(PaymentStatus.SUCCESS.name());
        payment.setAmount(order.getPayableAmount());
        payment.setRequestedAt(paidAt);
        payment.setPaidAt(paidAt);
        paymentMapper.insert(payment);
        ...
        history.setToStatus(OrderStatus.PAID.name());
        history.setChangedBy(userId);
        history.setChangeReason("mock payment success");
        orderStatusHistoryMapper.insert(history);
        sendPurchaseFeedback(userId, order.getId());
